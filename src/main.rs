...
fn main() -> Result<()> {
    ...
    let mut sggraph = SGGraph::from_serde(serde_json::to_value(stack_graph)?)?;
    sggraph.write_to_dot_file(&output_dot)?;

    println!(
        "Wrote stack graph JSON to {} and DOT to {}",
        output_path, output_dot
    );
